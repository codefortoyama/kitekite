 ...
 * 金額は2026年9月時点の公式運賃表を基にした、大人1名・普通席・片道の目安です。
 * ANAは2026年5月19日搭乗分から運賃体系が「シンプル／スタンダード／フレックス」の3区分に刷新されており、
 * スタンダードの金額はANA公式サイトの「スタンダード往復運賃表」（東京＝富山・エコノミークラス）を基にしています。
...
const FARES = [
  {
    ...
    plane: "ANAシンプル：約10,560〜29,260円（前日まで）／ANAスタンダード：約20,460〜52,470円（前日まで）／ANAフレックス：約32,560〜43,230円（当日可）",
    advantage: "train",
    point: "シンプル・スタンダードは前日までの購入が必要で、当日は購入できません。当日購入できるのはフレックスのみです。2026年5月19日の運賃改定でシンプル・フレックスの間に「スタンダード」（変更可・シンプルより高め）が新設されました。金額はいずれも空席連動の価格例の一つ。航空運賃は旅客施設使用料別。"
  },
  {
    pattern: "1〜3週間前に予約",
    train: "11,670円（トクだ値1）／9,080円（トクだ値14）",
    plane: "ANAシンプル：約10,560〜29,260円（空席連動）／ANAスタンダード：約20,240〜51,590円（28日前まで）",
    advantage: "even",
    point: "トクだ値1は前日、トクだ値14は14日前まで。いずれも列車・区間・席数限定です。ANAシンプル・スタンダードは空席状況により、予約期限が1・28・45日前のいずれかになります。"
  },
  {
    pattern: "45日以上前・早期購入",
    train: "9,080円（トクだ値14・30％引）",
    plane: "ANAシンプル：約10,120〜28,050円（45日前タイプ）／ANAスタンダード：約19,910〜50,600円（45日前まで）",
    advantage: "even",
